...
use generated::{
    ...
    MessageAddressTableLookup, MessageHeader, Reward, SanitizedMessage,
    SanitizedTransaction, SlotStatusEvent, TransactionEvent,
    TransactionStatusMeta, TransactionTokenBalance, UiTokenAmount,
...
impl From<V0LoadedMessage> for SerializableV0LoadedMessage {
    fn from(x: V0LoadedMessage) -> Self {
        Self {
            ...
            loaded_adresses: x
                .loaded_adresses
                .map(SerializableLoadedAddresses::from),
...
impl From<MessagePayload> for SerializableMessagePayload {
    fn from(x: MessagePayload) -> Self {
        match x {
            MessagePayload::Legacy(x) => {
                Self::Legacy(SerializableLegacyLoadedMessage::from(x))
            }
            MessagePayload::V0(x) => {
                Self::V0(SerializableV0LoadedMessage::from(x))
            }
        }
    }
}
...
impl From<SanitizedMessage> for SerializableSanitizedMessage {
    fn from(x: SanitizedMessage) -> Self {
        Self {
            message_payload: x
                .message_payload
                .map(SerializableMessagePayload::from),
        }
    }
}
...
impl From<TransactionTokenBalance> for SerializableTransactionTokenBalance {
    fn from(x: TransactionTokenBalance) -> Self {
        Self {
            ...
            ui_token_account: x
                .ui_token_account
                .map(SerializableUiTokenAmount::from),
            owner: x.owner,
        }
    }
}
...
impl From<TransactionEvent> for SerializableTransactionEvent {
    fn from(x: TransactionEvent) -> Self {
        Self {
            ...
            transaction: x
                .transaction
                .map(SerializableSanitizedTransaction::from),
